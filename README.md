...
### Script for Windows
Download [wake_server](wake_server.bat) and start it with double click or execute in terminal(make sure that cURL is installed)"
```
wake_server.bat
```
### Script for Linux/MAC
Download [wake_server](wake_server.sh) and execute the following commands:
```
